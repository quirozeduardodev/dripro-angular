...
import {BaseRepository, IBaseRepository} from "../database/repositories/base.repository";
...
import {BasicUserResponse, UserResponse} from "../types/response/user.response";
...
export class OfflineDataService {
  ...
  constructor(
    ...
    this.answerSync = new OfflineSynchronizeHandler<BasicAnswerResponse, Answer>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.answerRepository, () => this._answerEndpointService.basicAll(), 'answers', (item) => {return {id: item.id, userId: item.userId, type: item.reportType, folio: item.folio, countryId: item.countryId}});
    this.categorySync = new OfflineSynchronizeHandler<CategoryResponse, Category>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.categoryRepository, () => this._categoryEndpointService.all(), 'categories', (item) => {return {id: item.id, name: item.name}});
    this.contactSync = new OfflineSynchronizeHandler<ContactResponse, Contact>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.contactRepository, () => this._contactEndpointService.all(), 'contacts', (item) => {return {id: item.id, name: item.name}});
    this.countrySync = new OfflineSynchronizeHandler<CountryResponse, Country>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.countryRepository, () => this._countryEndpointService.all(), 'countries', (item) => {return {id: item.id, name: item.name}});
    this.customerSync = new OfflineSynchronizeHandler<CustomerResponse, Customer>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.customerRepository, () => this._customerEndpointService.all(), 'customers', (item) => {return {id: item.id, name: item.name}});
    this.delaySync = new OfflineSynchronizeHandler<DelayResponse, Delay>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.delayRepository, () => this._delayEndpointService.all(), 'delays', (item) => {return {id: item.id, name: item.name}});
    this.generatorSync = new OfflineSynchronizeHandler<GeneratorResponse, Generator>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.generatorRepository, () => this._generatorEndpointService.all(), 'generators', (item) => {return {id: item.id, name: item.name}});
    this.locationSync = new OfflineSynchronizeHandler<LocationResponse, Location>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.locationRepository, () => this._locationEndpointService.all(), 'locations', (item) => {return {id: item.id, name: item.name}});
    this.motorSync = new OfflineSynchronizeHandler<MotorResponse, Motor>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.motorRepository, () => this._motorEndpointService.all(), 'motors', (item) => {return {id: item.id, name: item.name}});
    this.QTASync = new OfflineSynchronizeHandler<QTAResponse, QTA>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.QTARepository, () => this._QTAEndpointService.all(), 'qtas', (item) => {return {id: item.id, name: item.name}});
    this.technicianSync = new OfflineSynchronizeHandler<BasicUserResponse, Technician>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.technicianRepository, () => this._userEndpointService.basicTechnicianAll(), 'technicians', (item) => {return {id: item.id, name: item.name}});
    this.typeSync = new OfflineSynchronizeHandler<TypeResponse, Type>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.typeRepository, () => this._typeEndpointService.all(), 'types', (item) => {return {id: item.id, name: item.name}});
    this.unitSync = new OfflineSynchronizeHandler<UnitResponse, Unit>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.unitRepository, () => this._unitEndpointService.all(), 'units', (item) => {return {id: item.id, name: item.name}});
    this.userSync = new OfflineSynchronizeHandler<BasicUserResponse, User>(this._unitOfWorkDatabase, this._unitOfWorkDatabase.userRepository, () => this._userEndpointService.basicAll(), 'users', (item) => {return {id: item.id, name: item.name}});
  }
...
export class OfflineSynchronizeHandler<TResponse, TEntity> {
  ...
  private _lastSynchronization$: BehaviorSubject<moment.Moment | null> = new BehaviorSubject<moment.Moment | null>(null);
  private _state$: BehaviorSubject<OfflineSynchronizeState> = new BehaviorSubject<OfflineSynchronizeState>('idle');
  ...
  constructor(unitOfWorkDatabase: UnitOfWorkDatabase, repository: IBaseRepository<TEntity>, functionData: () => Observable<TResponse[]>, keySynchronizationTable: string, functionDataMapper: (dataSource: TResponse) => TEntity) {
  ...
  public synchronize(): void {
    ...
    this._functionData().pipe(mergeMap<TResponse[], Observable<boolean>>(dataSource =>  {
      const nData: TEntity[] = dataSource.map<TEntity>(this._functionDataMapper);
      return this._repository.bulkUpsert(nData).pipe(map(insertedData => true), catchError(err => {
        console.log(err); return of(false);}));
    }), catchError(err => of(false))).subscribe(synchronizedSuccess => {
  ...
  private updateDataInfo(): void {
    forkJoin({
      ...
      lastSynchronization: this._unitOfWorkDatabase.synchronizationRepository.synchronizedAtByTable(this._keySynchronizationTable).pipe(take(1)),
      currentSynchronization: this._unitOfWorkDatabase.synchronizationRepository.updateSynchronizedAtByTable(this._keySynchronizationTable, moment()).pipe(take(1)),
    }).subscribe(result => {
      this._total$.next(result.count);
      this._lastSynchronization$.next(result.currentSynchronization || result.lastSynchronization);
    });
  }
}

export type OfflineSynchronizeState = 'idle' | 'synchronizing' | 'synchronizationFailed' | 'synchronized';
