...
export class ReportEndpointService extends EndpointService {
  ...
  public pagination(filters: ReportFiltersRequest,
    page: number = 1, limit: number = 10): Observable<PaginateResponse<BasicReportResponse>> {
  ...
  public save(request: CreateReportRequest): Observable<any> {
    ...
    return this.httpClient.post<any>(url, request, this.getOptions())
      .pipe(catchError((err: HttpErrorResponse) => this.handleError(err)),
        mergeMap(value => TypeBuilder.paginate(value, TypeBuilder.basicReports)));
  }
    ...
    return this.httpClient.get<any>(url, this.getOptions())
      .pipe(map(value =>  {
          if (value.photos) {
            ...
            return photos && Array.isArray(photos) ? (<any[]>photos).filter(value1 => value1) : []
          }
          return []
        }), catchError((err: HttpErrorResponse) => of([])));
  }

  public findAnswers(id: number): Observable<{type: string | null, answers: {[p: string]: any}}> {
    ...
    return this.httpClient.get<any>(url, this.getOptions())
      .pipe(catchError((err: HttpErrorResponse) => this.handleError(err)),
        mergeMap(value => {
          ...
          if (['jsa_sitio', 'jsa_field_generic', 'jsa_field_power_gen', 'jsa_field_marine', 'warehouse'].includes(type)) {
            return this.findImageByReportId(id)
              .pipe(map(photos => {
