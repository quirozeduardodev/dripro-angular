...
public class Bridge {
    ...
    private void loadWebView() {
        ...
        File dir = new File(context.getFilesDir(), "angularClients");
        if(!dir.exists()){
          dir.mkdir();
        }
        ...
        try {
          ...
          if (allFiles != null) {
            for (int i = 0; i < allFiles.length; i++) {
              File cur = allFiles[i];
              if (cur.isDirectory()) {
                String subDirName = cur.getName();
                if (isInteger(subDirName)) {
                  lastUnixTime = lastUnixTime < 0 || lastUnixTime < Integer.parseInt(subDirName) ? Integer.parseInt(subDirName) : lastUnixTime;
                }
              }
            }
          }
        ...
        if(lastUnixTime > 0) {
          localServer.hostFiles(dir.getPath() + "/" + lastUnixTime);
        } else  {
